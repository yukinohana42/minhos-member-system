...
要件書20.1の`DEC-01`〜`DEC-08`、`DEC-19`、`DEC-21`を確定し、Gate 1とGate 2 pre-entry boundaryが完了した場合に限り、要件書20.2の本番必須DECを未決のまま隔離testへ進める。Gate 2 runtime verificationはGate 3で行い、entry前へ循環させない。`G0-T`は`G0 Decision`の完了、production承認、または未決DECの受容を意味しない。
...
2. Ghost Admin API key、Stripe restricted key、Google Script Propertiesの保管区分、保管者role、アクセス者、最小scope、test/live分離、rotation・失効方法を確認する。秘密値の作成・入力は求めない。
3. `.env`、鍵ファイル、サービスアカウントJSONをコミットしない。`npm run check:secrets` を実行する。
4. フロントエンド、Sheet、テーマ、ログ、チャットへ秘密値が流れないことを確認する。

### Runtime verification（Gate 3）

test project作成後、責任者本人が秘密値をサービス／Apps Script画面へ直接入力する。Codexは値を取得・表示せず、存在、所有・アクセス権、test/live分離、失効可能性だけをread-backする。この確認はG0-T entry prerequisiteに含めない。

## Gate 3 — test mode / 検証環境

Gate 3へ入るには、完全なGate 0、または上記`G0-T`とGate 1およびGate 2 pre-entry boundaryの完了が必要である。Gate 2 runtime verificationはGate 3の中で行う。`G0-T`経由では上記の隔離条件をGate 3全体で維持する。
...
- [ ] 5つの永続triggerが0件であることを確認し、`manualSync()`を明示的に1回だけ実行した。実行時間上限時は一時`resumeSync` triggerだけを許可し、終了後に残存しないことを確認した。LockService、run lease、通知、backup／restoreもtest dataで確認した。
- [ ] 前項までのP1/P2が0で、責任者の別承認を得た場合だけ、`hourlySync`、`nightlySync`、`dailyBackup`、`monthlyBackup`、`onProfileFormSubmit`の5つの永続triggerをexact inventoryで確認した。
